defmodule StringParserTerrain do
  defmacro __using__(_opts) do
    quote do
      ...
      def findChar(c, level_vector) do
        {row, col} =
          Enum.reduce_while(level_vector, {0, nil}, fn line, {row, col} ->
            case Enum.find(line, fn elem -> elem == c end) do
              nil -> {:cont, {row + 1, col}}
      ...
      #  @spec start_pos(String.t()) :: Pos.t()
      def startPos(level_vector), do: findChar(~c"S", level_vector)

      #  @spec goal(String.t()) :: Pos.t()
      def goal(level_vector), do: findChar(~c"T", level_vector)
